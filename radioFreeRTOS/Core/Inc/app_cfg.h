/*
 * app_cfg.h
 *
 *  Created on: Apr 29, 2026
 *      Author: jdapo
 */

#ifndef INC_APP_CFG_H_
#define INC_APP_CFG_H_

#include "FreeRTOS.h"

/* Task Priorities */
#define SW_TASK_PRIO         (tskIDLE_PRIORITY)
#define CONTROLLER_TASK_PRIO (tskIDLE_PRIORITY + 1)

#define SENSOR_TASK_PRIO     (tskIDLE_PRIORITY + 2)
#define GPS_TASK_PRIO        (tskIDLE_PRIORITY + 2)

#define AGG_TASK_PRIO        (tskIDLE_PRIORITY + 3)
#define RADIO_TASK_PRIO      (tskIDLE_PRIORITY + 4)

/* Stack Sizes (words) */
#define SW_TASK_STACK         256
#define CONTROLLER_TASK_STACK 256

#define GPS_TASK_STACK        (512 * 4)
#define SENSOR_TASK_STACK     1024
#define AGG_TASK_STACK        256
#define RADIO_TASK_STACK      512


#endif /* INC_APP_CFG_H_ */
